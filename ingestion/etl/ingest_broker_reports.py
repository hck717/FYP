...
def load_broker_report_chunks(ticker: str, only_new: bool = False) -> int:
    ...
    pdf_files = sorted({*broker_dir.glob("*.pdf"), *broker_dir.glob("*.pdf.pdf")})
    ...
    existing_neo4j = _existing_source_files(ticker, "broker_report")
    existing_pg = _existing_postgres_doc_files(ticker, "broker_report")
    ...
    if not pdf_files:
        logger.info("[%s] No new broker report PDFs to ingest", ticker)
        return 0
    ...
    try:
        pg_written = _upsert_postgres_textual(ticker, "broker_report", all_docs, all_chunks)
        logger.info("[%s] Upserted %d broker report chunks to PostgreSQL", ticker, pg_written)
    except Exception as exc:
